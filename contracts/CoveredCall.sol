...
contract CoveredCall is ERC721 {
    uint public optionID; // keeps count of total options created and used as unique id
    ...
    event CoveredCallCreated(address _nftAddress, uint _nftID, uint _strikePrice, uint _expirationTime, uint _premiumPrice, uint _optionID, address _buyer, address _seller, uint _creationTime);
    ...
    function createCoveredCall(address _nftAddress, uint _nftID, uint _strikePrice, uint _expirationTime, uint _premiumPrice) public returns(uint) {
        ...
        require(_expirationTime >= 86400 * 7, "option expiration time must be greater than 7 days"); // 7 days minimum expiration
        ...
        emit CoveredCallCreated(_nftAddress, _nftID, _strikePrice, _expirationTime, _premiumPrice, optionID, option.buyer, msg.sender, block.timestamp);
        return(optionID);
    }
